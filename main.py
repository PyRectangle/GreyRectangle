...
select_pos_x = 0
select_pos_y = 0
start_select_pos_x = 0
start_select_pos_y = 0
animation_count = 0
drinking = False
selecting = False
drink_count = 0
start_select_set = False
delete_this = False
create_new = False
open_button = False
selected_image = 7
...
def animation_rect(block):
    global fps, animation_count
    rect = block.rect
    rect.height = 64
    animation_count += 1
    if animation_count >= fps / block.fps:
        animation_count = 0
        block.frame += 1
        rect.y = block.frame * 64
        if rect.y >= block.texture.get_height():
            rect.y = 0
            block.frame = 0
    else:
        return None
            ...
            if array_pos_of_block_x >= 0 and array_pos_of_block_y >= 0:
                try:
                    if region[array_pos_of_block_y][array_pos_of_block_x] == 8:
                        ...
                        if blocks[region[array_pos_of_block_y][array_pos_of_block_x]].animation:
                            win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                    ((block_x - menu_block_x) * 64 + menu_x,
                                                    (block_y - menu_block_y) * 64 + menu_y), blocks[region[array_pos_of_block_y][array_pos_of_block_x]].rect)
                        else:
                            win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                    ((block_x - menu_block_x) * 64 + menu_x,
                                                    (block_y - menu_block_y) * 64 + menu_y))
                    ...
                    try:
                        ...
                        if region[array_pos_of_block_y][array_pos_of_block_x] == 6:
                            ...
                            if blocks[region[array_pos_of_block_y][array_pos_of_block_x]].animation:
                                win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                        ((block_x - player_block_x) * 64 + player_x,
                                                        (block_y - player_block_y) * 64 + player_y), blocks[region[array_pos_of_block_y][array_pos_of_block_x]].rect)
                            else:
                                win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                        ((block_x - player_block_x) * 64 + player_x,
                ...
                try:
                    if region[array_pos_of_block_y][array_pos_of_block_x] == 8:
                        ...
                        if blocks[region[array_pos_of_block_y][array_pos_of_block_x]].animation:
                            win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                    ((block_x - editor_block_x) * 64 + editor_x,
                                                    (block_y - editor_block_y) * 64 + editor_y), blocks[region[array_pos_of_block_y][array_pos_of_block_x]].rect)
                        else:
                            win.window_surface.blit(blocks[region[array_pos_of_block_y][array_pos_of_block_x]].texture,
                                                    ((block_x - editor_block_x) * 64 + editor_x,
                                                    (block_y - editor_block_y) * 64 + editor_y))
                    color = (0, 0, 0)
                    width = 1
                    if test_rectangle_overlapping(array_pos_of_block_x - 1, array_pos_of_block_y - 1, array_pos_of_block_x, array_pos_of_block_y, start_select_pos_x, start_select_pos_y, select_pos_x, select_pos_y) or test_rectangle_overlapping(array_pos_of_block_x, array_pos_of_block_y - 1, array_pos_of_block_x - 1, array_pos_of_block_y, select_pos_x, start_select_pos_y, start_select_pos_x, select_pos_y):
                        if selecting or selected_blocks != []:
                            color = (0, 0, 255)
                            width = 2
                    pygame.draw.polygon(win.window_surface, color,
                                        (((block_x - editor_block_x) * 64 + editor_x,
                                          (block_y - editor_block_y) * 64 + editor_y),
...
def test_rectangle_overlapping(x1, y1, x2, y2, x3, y3, x4, y4):
    return x1 <= x3 and y1 <= y3 and  x2 >= x4 and y2 >= y4 or x1 >= x3 and y1 >= y3 and  x2 <= x4 and y2 <= y4
...
def move_editor():
    ...
    if win.keys[pygame.K_RIGHT] or win.keys[pygame.K_d]:
        editor_x -= 1
        if editor_x < -opened_level[1][0] * 64 + 320:
            editor_x = -opened_level[1][0] * 64 + 320
    if win.keys[pygame.K_LEFT] or win.keys[pygame.K_a]:
        editor_x += 1
        if editor_x > 320:
            editor_x = 320
    if win.keys[pygame.K_UP] or win.keys[pygame.K_w]:
        editor_y += 1
        if editor_y > 240:
            editor_y = 240
    if win.keys[pygame.K_DOWN] or win.keys[pygame.K_s]:
        editor_y -= 1
...
class Block:
    def __init__(self, texture, solid, dead, colorkey, animation, fps):
...
blocks = [
    Block(None, False, False, None, False, 0),
    Block(images[1], True, False, None, False, 0),
    Block(images[2], True, False, None, False, 0),
    Block(images[3], True, False, None, False, 0),
    Block(images[4], False, False, (0, 0, 0), False, 0),
    Block(images[5], False, True, None, False, 0),
    Block(images[6], False, False, (255, 255, 255), False, 0),
    Block(images[9], True, False, None, False, 0),
    Block((images[10], images[11]), False, False, None, False, 0),
    Block(images[12], False, False, (255, 255, 255), False, 0),
    Block(images[13], False, False, (255, 255, 255), False, 0),
    Block(images[14], False, False, (255, 255, 255), False, 0),
    Block(images[15], True, False, None, False, 0),
    Block(images[16], True, False, None, False, 0),
    Block(images[17], False, False, (255, 255, 255), True, 8)
]
...
while True:
    ...
    if in_level:
        ...
        for i in range(speed):
            if win.keys[pygame.K_RIGHT] and test_walls("right") or win.keys[pygame.K_d] and test_walls("right"):
                player_x -= 1
            if win.keys[pygame.K_LEFT] and test_walls("left") or win.keys[pygame.K_a] and test_walls("left"):
                player_x += 1
    ...
    if in_editor:
        ...
        if level_resize:
            ...
            for i in range(speed):
                move_editor()
            if win.mouse_pressed[0] and not win.keys[pygame.K_LSHIFT] and escape_count == 0:
                mouse_pos = win.get_mouse_pos()
                try:
                    ...
                    pass
            if win.mouse_pressed[2] and not win.keys[pygame.K_LSHIFT] and escape_count == 0:
                mouse_pos = win.get_mouse_pos()
                try:
                    array_pos_x = int((-editor_x + mouse_pos[0]) / 64)
                    array_pos_y = int((-editor_y + mouse_pos[1]) / 64)
                    ...
                    pass
            if win.mouse_pressed[1] and mouse_pos != win.get_mouse_pos() and not win.keys[pygame.K_LSHIFT]:
            ...
            if win.keys[pygame.K_F6]:
                opened_level[2] = -int((win.get_mouse_pos()[0] - editor_x) / 64 - 4) * 64 + 32
                opened_level[3] = -int((win.get_mouse_pos()[1] - editor_y) / 64 - 4) * 64 - 40
            ...
            if win.keys[pygame.K_LSHIFT] and win.mouse_pressed == (1, 0, 0):
                selecting = True
                if not start_select_set:
                    start_select_set = True
                    start_select_pos_x = int((win.get_mouse_pos()[0] - editor_x) / 64) - 1
                    start_select_pos_y = int((win.get_mouse_pos()[1] - editor_y) / 64) - 1
                select_pos_x = int((win.get_mouse_pos()[0] - editor_x) / 64)
                select_pos_y = int((win.get_mouse_pos()[1] - editor_y) / 64)
            else:
                if start_select_set:
                    selected_blocks = []
                    array = []
                    blocks_x = select_pos_x - start_select_pos_x
                    blocks_y = select_pos_y - start_select_pos_y
                    start_x = start_select_pos_x
                    start_y = start_select_pos_y
                    if blocks_x < 0:
                        blocks_x -= 2
                        start_x += blocks_x + 1
                        blocks_x = -blocks_x
                    ...
                    for y in range(blocks_y):
                        for x in range(blocks_x):
                            try:
                                if not start_y + y + 1 < 0 and not start_x + x + 1 < 0:
                                    array.append(region[start_y + y + 1][start_x + x + 1])
                            except IndexError:
                                pass
                        selected_blocks.append(array)
                        array = []
                    select_options = True
                    win.create_button(10, 30, 520, 50, "Fill with selected block", 45)
                    win.create_button(10, 100, 260, 50, "Fill with air", 45)
                    win.create_button(10, 170, 115, 50, "Copy", 42)
                    win.create_button(10, 400, 200, 50, "Deselect", 45)
                start_select_set = False
                selecting = False
            if win.keys[pygame.K_RSHIFT] and not win.mouse_pressed == (1, 0, 0):
                selected_blocks = []
            if win.keys[pygame.K_v]:
                paste_x = int((win.get_mouse_pos()[0] - editor_x) / 64)
                paste_y = int((win.get_mouse_pos()[1] - editor_y) / 64)
                for y in range(blocks_y):
                    for x in range(blocks_x):
                        try:
                            if not start_y + y + 1 < 0 and not start_x + x + 1 < 0:
                                region[paste_y + y][paste_x + x] = region[start_y + y + 1][start_x + x + 1]
                        except IndexError:
                            pass
        if select_options:
            if win.button == "Fill with selected block":
                for y in range(blocks_y):
                    for x in range(blocks_x):
                        try:
                            if not start_y + y + 1 < 0 and not start_x + x + 1 < 0:
                                region[start_y + y + 1][start_x + x + 1] = selected_block
                        except IndexError:
                            pass
            if win.button == "Fill with air":
                for y in range(blocks_y):
                    for x in range(blocks_x):
                        try:
                            if not start_y + y + 1 < 0 and not start_x + x + 1 < 0:
                                region[start_y + y + 1][start_x + x + 1] = 0
                        except IndexError:
                            pass
            if win.button == "Copy":
                escape_count = fps / 2
                ...
                for i in range(4):
                    win.remove_button(0)
            if win.button == "Deselect" or win.keys[pygame.K_ESCAPE]:
                escape_count = fps / 2
                select_options = False
                selected_blocks = []
                win.button = None
